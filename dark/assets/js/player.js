$(function() {
    ...
    function showHover(a) {
        ...
        seekT = a.clientX - seekBarPos.left;
    ...
    function updateCurrTime() {
        ...
        else tProgress.text(curMinutes + ':' + curSeconds); if (isNaN(durMinutes) || isNaN(durSeconds)) tTime.text('00:00');
        else tTime.text(durMinutes + ':' + durSeconds); if (isNaN(curMinutes) || isNaN(curSeconds) || isNaN(durMinutes) || isNaN(durSeconds)) trackTime.removeClass('active');
    ...
    function selectTrack2(c) {
        $.getJSON('https://api.uomg.com/api/rand.music?', {
            mid: '7250343074',
            format: 'json'
        }, function(a, b) {
            if (a.code == 1) {
                if (c == 0) i.attr('class', 'fa fa-play');
                else {
                    albumArt.removeClass('buffering');
                    i.attr('class', 'fa fa-pause')
                }
                ...
                tTime.text('00:00');
                currAlbum = a.data.artistsname;
                currTrackName = a.data.name;
                currArtwork = a.data.picurl.replace(/^http:\/\//i, 'https://');
                audio.src = a.data.url.replace(/^http:\/\//i, 'https://');
                nTime = 0;
                bTime = new Date();
                bTime = bTime.getTime();
                if (c != 0) {
    ...
    function showmessage() {
        if (!audio.paused) $.NZ_MsgBox.toast({
            content: "当前播放：" + currTrackName + " - " + currAlbum,
            location: "top",
            showtime: 2500
        })
    }
